...
The right response was to remove the stale policy path and preserve only the live behavior that still mattered. The maintainer formally requested that change. After the requested code and documentation corrections were addressed, the maintainer re-verified the final head, approved it, and the PR was merged into `main` on August 24, 2026. That merge establishes upstream acceptance of this scoped correction and the live `DatasetLookup` hardening; it does not turn the earlier dead-code premise into a validated product boundary or imply broader endorsement.

On [Rosetta PR #320](https://github.com/griddynamics/rosetta/pull/320), a maintainer independently pressure-tested a performance optimization across millions of inputs, reported zero divergences, and reproduced the speed improvement. The same review requested targeted cross-line and same-segment regression coverage plus explanatory invariants; those changes were subsequently added, independently re-verified, and approved. The PR was merged into `main` on August 24, 2026. The strongest accurate lifecycle claim is therefore **independently corroborated, requested changes addressed, approved, and merged upstream**—not broader adoption or endorsement.
